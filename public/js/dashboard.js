import {
  db, storage, collection, doc, addDoc, getDocs, updateDoc, deleteDoc,
  ref, getDownloadURL, serverTimestamp,
} from "./firebase-init.js";
import { requireTeacherPage } from "./nav.js";
import { guardConfig, toast, escapeHtml, todayStr, daysBetween, STATUS_LABEL, REASON_TYPES } from "./utils.js";

if (!guardConfig()) {
  requireTeacherPage("dashboard").then(({ cls }) => init(cls.id));
}
...
async function init(classId) {
  ...
  async function loadAll() {
    const [stuSnap, absSnap] = await Promise.all([
      getDocs(collection(db, "classes", classId, "students")),
      getDocs(collection(db, "classes", classId, "absences")),
    ]);
    students = [];
    ...
    absences.sort((a, b) => (a.startDate < b.startDate ? 1 : -1));
    renderStats();
    renderCalendar();
  ...
  function renderStats() {
    ...
    const waiting = absences.filter((a) => a.status === "서류대기").length;
    const done = absences.filter((a) => a.status === "제출완료" || a.status === "확인완료").length;
    const overdue = absences.filter((a) => a.status === "서류대기" && daysBetween(a.startDate, todayStr()) > 5).length;
    document.getElementById("stat-grid").innerHTML = `
      <div class="stat-box"><div class="num">${students.length}</div><div class="lbl">전체 학생</div></div>
      <div class="stat-box"><div class="num">${total}</div><div class="lbl">전체 결석 기록</div></div>
  ...
  function dateAbsenceMap() {
    const map = {};
    for (const a of absences) {
      if (!a.startDate || !a.endDate) continue;
      const cur = new Date(a.startDate + "T00:00:00");
  ...
  function renderDateList() {
    ...
    titleEl.textContent = `${selectedDate} 결석 학생`;
    const map = dateAbsenceMap();
    const items = map[selectedDate] || [];
    if (!items.length) {
      listEl.innerHTML = `<div class="empty">이 날짜에 등록된 결석 기록이 없습니다.</div>`;
      return;
    }
    listEl.innerHTML = renderAbsenceTable(items);
  ...
  document.getElementById("btn-add-manual").onclick = () => {
    ...
    area.innerHTML = `
      <div class="card" style="background:var(--bg)">
        ...
        <div class="field"><label>결석 유형</label>
          <select id="m-type"><option value="">미정</option>${REASON_TYPES.map((t) => `<option value="${t}">${t}</option>`).join("")}</select>
        </div>
    ...
    document.getElementById("m-save").onclick = async () => {
      ...
      await addDoc(collection(db, "classes", classId, "absences"), {
  ...
  function renderStudentSummary() {
    ...
    for (const s of students) {
      const mine = absences.filter((a) => a.studentId === s.id);
      const waiting = mine.filter((a) => a.status === "서류대기").length;
      const latest = mine[0];
  ...
  function renderStudentDetail() {
    ...
    const mine = absences.filter((a) => a.studentId === expandedStudentId);
    area.innerHTML = `<h2 style="margin-top:20px">${escapeHtml(s.name)} 학생 결석 기록</h2>` +
      (mine.length ? renderAbsenceTable(mine) : `<div class="empty">결석 기록이 없습니다.</div>`);
    bindRowActions(area);
  }

  // ---------- 미제출자 보기 ----------
  function renderPending() {
    const list = absences.filter((a) => a.status === "서류대기").sort((a, b) => (a.startDate < b.startDate ? -1 : 1));
    const el = document.getElementById("pending-list");
    if (!list.length) { el.innerHTML = `<div class="empty">미제출 서류가 없습니다.</div>`; return; }
    el.innerHTML = renderAbsenceTable(list, true);
    bindRowActions(el);
  }
  ...
  function renderAbsenceTable(items, showOverdue) {
    const todayS = todayStr();
    const rows = items.map((a) => {
      const overdue = a.status === "서류대기" && daysBetween(a.startDate, todayS) > 5;
      return `
        <tr>
          <td>${escapeHtml(a.startDate)}${a.endDate && a.endDate !== a.startDate ? " ~ " + escapeHtml(a.endDate) : ""}</td>
          <td>${escapeHtml(a.studentNumber || "")}번 ${escapeHtml(a.studentName || "")}</td>
          <td>${escapeHtml(a.absenceType || "-")}</td>
          <td>${statusBadge(a.status)}${showOverdue && overdue ? ' <span class="badge badge-rejected">기한초과</span>' : ""}</td>
          <td class="row-actions" style="white-space:nowrap">
            ${a.pdfPath ? `<button class="btn btn-ghost btn-sm" data-view-pdf="${a.id}">PDF</button>` : ""}
            ${a.status === "제출완료" ? `<button class="btn btn-secondary btn-sm" data-approve="${a.id}">확인완료</button><button class="btn btn-danger btn-sm" data-reject="${a.id}">반려</button>` : ""}
            <button class="btn btn-ghost btn-sm" data-del="${a.id}">삭제</button>
          </td>
        </tr>`;
    }).join("");
  ...
  function bindRowActions(root) {
    root.querySelectorAll("[data-view-pdf]").forEach((btn) => {
      btn.onclick = async () => {
        const a = absences.find((x) => x.id === btn.dataset.viewPdf);
        const url = await getDownloadURL(ref(storage, a.pdfPath));
        window.open(url, "_blank");
      };
    });
    root.querySelectorAll("[data-approve]").forEach((btn) => {
      btn.onclick = async () => {
        await updateDoc(doc(db, "classes", classId, "absences", btn.dataset.approve), { status: "확인완료", updatedAt: serverTimestamp() });
        toast("확인 처리되었습니다.");
    ...
    root.querySelectorAll("[data-reject]").forEach((btn) => {
      btn.onclick = async () => {
        ...
        await updateDoc(doc(db, "classes", classId, "absences", btn.dataset.reject), { status: "반려", updatedAt: serverTimestamp() });
        toast("반려되었습니다.");
        await loadAll(); renderDateList();
      };
    });
    root.querySelectorAll("[data-del]").forEach((btn) => {
      btn.onclick = async () => {
        if (!confirm("이 결석 기록을 삭제할까요?")) return;
        await deleteDoc(doc(db, "classes", classId, "absences", btn.dataset.del));
        toast("삭제되었습니다.");
        await loadAll(); renderDateList();
      };
    });
