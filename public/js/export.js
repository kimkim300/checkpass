...
import { guardConfig, toast, escapeHtml, todayStr, STATUS_LABEL } from "./utils.js";
...
async function init(classId) {
  ...
  async function search() {
    const snap = await getDocs(collection(db, "classes", classId, "absences"));
    const all = [];
  ...
  function renderResults() {
    ...
    if (!currentResults.length) {
      ...
      el.innerHTML = `<div class="roster-table-wrap"><table>
        <thead><tr><th>날짜</th><th>학생</th><th>유형</th><th>상태</th></tr></thead>
  ...
  document.getElementById("e-merge").onclick = async () => {
    ...
    try {
      ...
      a.download = `결석계_수합_${start.value}_${end.value}.pdf`;
      a.click();
  ...
  document.getElementById("e-cleanup").onclick = async () => {
    ...
    for (const a of currentResults) {
      try {
        ...
        await updateDoc(doc(db, "classes", classId, "absences", a.id), { pdfPath: null, photoPath: null });
        ok++;
      } catch (err) {
        console.error(err);
      }
